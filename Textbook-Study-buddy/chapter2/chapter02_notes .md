...
## 12. Google Colab Notes

Each chapter has a companion notebook — open directly in Colab, no setup required:

| Chapter | Colab Link |
|---------|-----------|
| Ch 2 | https://colab.research.google.com/github/fchollet/deep-learning-with-python-notebooks/blob/master/chapter02_mathematical-building-blocks.ipynb |
| All chapters | https://github.com/fchollet/deep-learning-with-python-notebooks |

### Colab Tips Worth Knowing

**Enable GPU runtime** — required for later chapters (Ch 8+), good habit to set now:  
`Runtime → Change runtime type → Hardware accelerator → T4 GPU`

**Cells don't auto-save state** — if your session disconnects, re-run all cells from the top.  
Use `Runtime → Run all` to restore state quickly.

**Add your own experiment cells** — insert a new cell after any example and modify it.  
Changing one parameter and re-running is the fastest way to build intuition.

**Print shapes everywhere while learning:**
```python
# Add this pattern after any tensor operation
print(f"shape: {x.shape}, dtype: {x.dtype}, ndim: {x.ndim}")
```

**Save your work** — Colab notebooks auto-save to Google Drive.  
`File → Save a copy in Drive` to keep a personal version with your own notes/cells.

> **Teaching tip:** For class demos, Colab is ideal — students follow along in their  
> own browser with zero install. Share the Chollet notebook link and have them  
> `Save a copy in Drive` before modifying anything.
