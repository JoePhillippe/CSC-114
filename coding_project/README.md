...
| `grading_agentV5.py` | Renders PDFs as images and submits batch grading job |
...
Canvas (manual download)
  ...
  intakeV2.py               ← interactive intake script
   ...
   queue/                   ← renamed files with consistent naming
        ↓
  grading_agentV5.py        ← renders PDFs, submits batch to Claude API
        ↓
...
- Supports chaining: run reprocessV1.py then grading_agentV5.py
