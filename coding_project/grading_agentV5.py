"""
grading_agentV5.py  —  Automated Grading Agent (Batch Submission)
Cisco Cyber Operations Grading System

Changes from V4:
  - Batch API submission — 50% cost reduction
  - Template answer extraction — 89% token reduction on template input
  - Switched to claude-sonnet-4-6 — 40% cost reduction vs Opus
  - Combined cost reduction: ~90% vs V4
  - Submits all queue files as one batch job
  - Saves batch ID to batch_jobs.csv for retrieval
  - Run batch_retrieveV1.py next day to collect results and write feedback

Usage:
    python grading_agentV5.py    <- submits batch, exits immediately
