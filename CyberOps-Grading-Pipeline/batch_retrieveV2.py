"""
batch_retrieveV2.py  —  Batch Results Retrieval Script (Continuous Mode)
...
Runs continuously — checks for completed batch results every 24 hours.
Can also be run once manually — already processed results are skipped.
...
AGENT_VERSION = "V5"
POLL_HOURS    = 1     # How often to check for completed batches
# ──────────────────────────────────────────────────────────────────────────────
...
def process_pending_batches(client, course_dir: Path) -> int:
    """Check all pending batches and write feedback for completed ones.
    Returns number of feedback documents written this cycle."""
    ...
    for batch_id in batch_ids:
        ...
        if batch.processing_status != "ended":
            print(f"  Not ready yet — will check again in {POLL_HOURS} hours.")
            continue
...
def main():
    ...
    print(f"  Continuous Mode  \u2014  checks every {POLL_HOURS} hours")
    print("="*54)

    course_dir = pick_course_folder()
    print(f"\n  Working in:  {course_dir.name}")

    # Check for pending jobs before starting loop
    pending_jobs = load_batch_manifest(course_dir)
    if not pending_jobs:
    ...
    print(f"\n  Found {len(pending_jobs)} pending result(s).")
    print(f"  Checking every {POLL_HOURS} hours until all complete.")
    ...
    try:
        while True:
            print(f"  [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]  "
                  f"Checking for completed batches...")

            written        = process_pending_batches(client, course_dir)
            total_written += written

            print(f"\n  Cycle complete.  {written} new feedback document(s) written.")
            print(f"  Total written this session: {total_written}")

            # Stop automatically when all jobs are done
            remaining = load_batch_manifest(course_dir)
            if not remaining:
                print(f"\n  All batch jobs complete.  No more pending results.")
                print(f"  Stopping.\n")
                break

            print(f"\n  {len(remaining)} job(s) still pending.")
            print(f"  Next check at: "
                  f"{datetime.fromtimestamp(time.time() + POLL_HOURS * 3600).strftime('%Y-%m-%d %H:%M')}")
            print(f"  Press Ctrl+C to stop early.\n")
            time.sleep(POLL_HOURS * 3600)

    except KeyboardInterrupt:
        print(f"\n\n  Stopped by user.")
        print(f"  Total feedback documents written: {total_written}")
        print(f"  Run again to continue retrieving pending results.\n")
