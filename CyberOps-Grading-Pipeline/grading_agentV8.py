1"""
grading_agentV8.py  —  Automated Grading Agent (Batch Submission)
Cisco Cyber Operations Grading System

Changes from V7:
  - Adjacent page screenshot detection added to grading prompt
  - Handles student submission pattern where screenshot is pasted on the
    page following the yellow instruction box rather than on the instruction
    page itself — now checks instruction page, page before, and page after
  - Fixes incorrect missing screenshot deductions for this submission style

Usage:
    python grading_agentV8.py    <- submits batch, exits immediately
    python batch_retrieveV2.py   <- collects results, checks hourly
...
MAX_BATCH_MB  = 200    # split batches above this size — safe margin below 256MB API limit
# ──────────────────────────────────────────────────────────────────────────────
...
SCREENSHOT LOCATION \u2014 CHECK ADJACENT PAGES BEFORE MARKING MISSING
  Students submit labs using a Word template with a yellow instruction box
  and a formatted area below it that can extend to the next page.
  Some students paste their screenshot into this formatted area which
  may appear on the PAGE IMMEDIATELY FOLLOWING the instruction page.
...
TYPE 1 \u2014 SCREENSHOT IS THE ANSWER
  ...
  Grade screenshot directly against the answer template.
  Check all three adjacent pages before marking missing.
  If missing after checking all pages deduct full points.
...
TYPE 2 \u2014 SCREENSHOT REQUIRED BUT NOT THE ANSWER
  ...
  STEP A: Verify screenshot exists — check instruction page AND adjacent pages.
    If MISSING after checking all three pages:
      Flag as separate deduction, still grade text answers,
      warn instructor may not accept answers without screenshot.
    If PRESENT on any of the three pages:
      Do NOT use screenshot content to grade text answers.
...
def estimate_size_mb(pages: list) -> float:
    """Estimate size in MB for one student submission based on rendered pages."""
    from io import BytesIO
    total = 0
    for page in pages:
        buf = BytesIO()
...
def main():
    ...
    for file in pending:
        ...
        # Estimate size for batch splitting
        size_mb = estimate_size_mb(pages) if pages else 3.8
    ...
    # Auto-split into batches if total size exceeds MAX_BATCH_MB
    # Group requests into chunks that stay under the size limit
    ...
    for req, log in zip(batch_requests, submissions_log):
        student_mb = log.get("size_mb", 3.8)
        if current_chunk and (current_mb + student_mb) > MAX_BATCH_MB:
            chunks.append((list(current_chunk), list(current_log)))
            current_chunk = []
            current_log   = []
            current_mb    = 0.0
        current_chunk.append(req)
        current_log.append(log)
        current_mb += student_mb

    if current_chunk:
        chunks.append((current_chunk, current_log))

    total_chunks = len(chunks)
    print(f"  Splitting into {total_chunks} batch(es) to stay under {MAX_BATCH_MB}MB each.")

    # Submit each chunk as a separate batch
    for chunk_num, (chunk_requests, chunk_log) in enumerate(chunks, start=1):
        est_mb = sum(s.get("size_mb", 3.8) for s in chunk_log)
        print(f"\n  {'='*54}")
        print(f"  Submitting batch {chunk_num}/{total_chunks} — "
              f"{len(chunk_requests)} student(s) — ~{est_mb:.0f}MB")
        print(f"  {'='*54}")

        try:
            batch = client.messages.batches.create(requests=chunk_requests)
        except Exception as e:
            print(f"\n  ERROR: Batch {chunk_num} submission failed — {e}\n")
            print(f"  Skipping this batch — other batches will still submit.")
            continue

        batch_id = batch.id
        print(f"  ✓  Batch {chunk_num} submitted successfully.")
        print(f"  Batch ID:  {batch_id}")
        print(f"  Status:    {batch.processing_status}")

        save_batch_manifest(course_dir, batch_id, chunk_log)
