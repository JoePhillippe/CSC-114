...
- Supports chaining: run reprocessV1.py then grading_agentV8.py
...
### Adjacent Page Screenshot Detection — Why V8 Was Needed
During the pilot semester a grading accuracy issue was discovered during manual review of a student submission. The student had completed every screenshot correctly but received zero points for eleven screenshots — all marked as missing by the grading agent.

The root cause was how students paste screenshots into the Word lab template. The template provides a yellow instruction box followed by a formatted paste area below it. When a student pastes a large screenshot into this area it can extend onto the next page of the document. When the PDF is rendered the screenshot appears on the page immediately following the yellow instruction page rather than on the instruction page itself.

The grading agent was checking only the instruction page for a screenshot. Finding only the yellow instruction text it marked the screenshot as missing even though the screenshot was clearly visible on the adjacent page.

**The fix in V8** adds an explicit three-page check rule to the grading prompt:

```
BEFORE marking any screenshot as missing check ALL three locations:
  1. The instruction page itself
  2. The page immediately BEFORE the instruction page
  3. The page immediately AFTER the instruction page

A screenshot found on ANY of these three pages counts as PRESENT.
Only mark MISSING if NONE of the three pages contain a screenshot.
Do NOT penalize a student for pasting on the adjacent page.
```

This is a prompt-only change. No code logic was modified. The PDF pages are already sent as sequential images so Claude has full visibility of adjacent pages. The prompt simply needed to instruct Claude to look at them before making a missing screenshot determination.

This issue was caught because the pilot semester uses manual verification of all graded results before uploading to Canvas. The student received the correct grade. Without manual verification this error would have gone undetected and resulted in a significantly lower grade than deserved.

### Batch Size Management — Why Auto-Split Was Needed
The Anthropic Batch API rejects requests larger than 256MB with a 413 error. Each student PDF rendered at 100 DPI JPEG quality 70 produces approximately 3.8MB of image data. This means:

| Submissions | Estimated Size | Result |
|---|---|---|
| 29 students, 1 lab | ~109MB | Fits in one batch |
| 52 students, mix of labs | ~197MB | Fits in one batch |
| 73 students, 2 labs | ~277MB | Exceeds 256MB — rejected |

The 73-submission failure occurred when two labs were queued simultaneously — lab 4 and lab 5 regraded together. The batch submission was rejected before a single student was graded.

The fix in V6 adds three components:

**`estimate_size_mb()` function** — measures the actual rendered JPEG size for each student submission after pdf2image conversion rather than estimating from page count alone.

**`MAX_BATCH_MB = 200` constant** — a safe threshold with 56MB margin below the 256MB API limit. Adjustable at the top of the script if needed.

**Auto-split loop** — groups submissions into chunks that stay under 200MB and submits each chunk as a separate batch job. Each batch gets its own ID tracked in `batch_jobs.csv`. The retrieval script handles multiple batch IDs automatically with no changes needed.

The terminal output shows progress clearly:
```
Calculating batch sizes...
Splitting into 2 batch(es) to stay under 200MB each.

Submitting batch 1/2 — 52 student(s) — ~197MB
✓  Batch 1 submitted.  ID: msgbatch_abc123

Submitting batch 2/2 — 21 student(s) — ~80MB
✓  Batch 2 submitted.  ID: msgbatch_def456
```
