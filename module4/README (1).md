...
## K-fold Experiment — k=4 vs k=5 Comparison

### What was changed

One line in Step 5 and one line in Step 6:
...
Everything else — the loop, the slicing, the averaging — adjusts automatically
because the code uses `k` as the variable throughout.

With k=5: `num_val_samples = 480 // 5 = 96 samples per fold` (vs 120 with k=4).

> **Note:** Step 6 in the k=5 run only processed 4 folds — the `k = 4` line
> in that cell was not updated. The validation MAE curves are therefore based
> on 4 folds, not 5. The Step 7 plots reflect this.

---

### Step 5 Results — k=5 (50 epochs)
...
MAE per fold : [0.275, 0.307, 0.348, 0.239, 0.319]
Average MAE  : 0.298
Avg dollars off: $29758
```

---

### Step 7 Results — k=5 Validation MAE Curves

**Plot 1 (all 200 epochs):**
- MAE drops sharply from ~1.0 through epoch ~25
- Flattens around 0.30 for the remainder — same shape as k=4 run

**Plot 2 (epoch 10 onward, zoomed):**
- Continues falling steeply through epoch ~50
- Bottoms around 0.28–0.29, stays flat and noisy through epoch 200
- No sharp upward turn — model plateaus, safe epoch count remains ~130

---

### Step 8 Results — k=5 Final Model

```
Test MSE        : 0.3206
Test MAE        : 0.3270
Dollars off avg : $32699
```

---

### Step 9 Results — k=5 Predictions

```
First 5 predictions vs actual prices:
   Predicted        Actual        Off by
------------------------------------------
$   245,744  $   218,800  $    26,944
$   191,653  $   218,400  $    26,747
$   112,633  $    93,800  $    18,833
$   227,864  $   173,400  $    54,464
$   230,248  $   229,700  $       548
```
...
#### K-fold validation scores (Step 5)

| Fold | k=4 MAE | k=5 MAE |
|---|---|---|
| Fold 1 | 0.302 | 0.275 |
| Fold 2 | 0.299 | 0.307 |
| Fold 3 | 0.246 | 0.348 |
...
| Fold 5 | — | 0.319 |
| **Average** | **0.291** | **0.298** |
| **Dollars off** | **$29,099** | **$29,758** |

> The k=5 fold scores spread wider (0.239 to 0.348) than k=4 (0.246 to 0.317).
> With only 96 validation samples per fold vs 120, each individual score is noisier.

#### Final model test results (Step 8)

| Metric | k=4 | k=5 | Difference |
|---|---|---|---|
| Test MSE | 0.2859 | 0.3206 | +0.0347 worse |
| Test MAE | 0.2944 | 0.3270 | +0.0326 worse |
| Dollars off avg | $29,441 | $32,699 | +$3,258 worse |

#### Sample predictions (Step 9)

| District | Actual | k=4 Predicted | k=4 Off by | k=5 Predicted | k=5 Off by |
|---|---|---|---|---|---|
| 1 | $218,800 | $250,011 | $31,211 | $245,744 | $26,944 |
| 2 | $218,400 | $182,687 | $35,713 | $191,653 | $26,747 |
| 3 | $93,800  | $115,255 | $21,455 | $112,633 | $18,833 |
| 4 | $173,400 | $211,774 | $38,374 | $227,864 | $54,464 |
| 5 | $229,700 | $203,813 | $25,887 | $230,248 | $548    |

---

### What the experiment showed

**More folds did not improve predictions.** The k=5 final model was
$3,258 worse on average than k=4 ($32,699 vs $29,441).

The reason is that K-fold is a **measurement tool**, not a training technique.
It finds a reliable validation score and a safe epoch count — but the final
model is trained fresh from random weights. That random starting point
is what caused the difference between runs, not the number of folds.

Individual prediction results tell a mixed story — k=5 was closer on
districts 1, 2, 3, and 5, but badly missed district 4 ($54,464 off vs $38,374).
That single outlier pulled the overall average up.

**What actually limits accuracy here is the dataset size** — 480 training
samples is the real ceiling. Neither k=4 nor k=5 can overcome that.

---

*K=5 experiment run on the same Colab notebook:*
*https://colab.research.google.com/drive/1dCTCEiQxPXmX0LCC3NvFztS58lobaojd*
